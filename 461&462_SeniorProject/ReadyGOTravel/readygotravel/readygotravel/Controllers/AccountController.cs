    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterAccountViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                var checkUser = userDB.Users
                    .Where(u => u.Email == model.User.Email)
                    .Select(e => e).FirstOrDefault();
                if(checkUser != null)
                {
                    //Error Message to be put on the page.
                    ViewBag.Error = "The email is already in use. Please use a different one.";
                    return View();
                }
                ...
                if (result.Succeeded)
                {
                    ...
                    //Create a Person with the Info we have.
                    Person Person = new Person { UserID = user.Id, FirstName = model.FirstName, LastName = model.LastName };
                    db.People.Add(Person); //Add person to the DB
